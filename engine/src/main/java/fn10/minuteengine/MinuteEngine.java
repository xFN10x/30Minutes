...
public final class MinuteEngine {
    ...
    public int loadGame(String op, String... args) {
        URL gameInfoJsonUrl;
        Path jarbinPath = Path.of(args[0]);
        //get the game info json
        try {
            if (op.equals("jar"))
                gameInfoJsonUrl = MinuteJarUtils.getFileFromJar(jarbinPath, "me.game.json");
            else if (op.equals("bin")) {
                if (args.length < 2)
                {
                    logger.error("Expected 2 arguments for 'bin' op, but got 1.");
                    return ERR_GAME_FAIL_LOAD_URL_ERROR;
                }
                String resFolderString = args[1];
                Path resFolder = Path.of(resFolderString);
                gameInfoJsonUrl = resFolder.resolve("me.game.json").toUri().toURL();
            } else
                return ERR_GAME_FAIL_LOAD_URL_ERROR;
        } catch (MalformedURLException e) {
            logger.log(Level.ERROR, "Failed to get jar URL with op: " + op + ", and path " + jarbinPath + ".", e);
            return ERR_GAME_FAIL_LOAD_URL_ERROR;
        }
        ...
        try {
            loader = new URLClassLoader(new URL[]{jarbinPath.toUri().toURL()});
        } catch (MalformedURLException e) {
            e.printStackTrace();
            return ERR_GAME_FAIL_LOAD_URL_ERROR;
        }
        ...
        try (InputStream jsonStream = loader.getResourceAsStream("me.game.json")) {
    ...
    static void main(String[] args) {
        ...
        if (args.length == 0) {
            logger.info("Expected at least 2 args:\n bin [pathToClasses] [pathToResources]\n jar [pathToJar]");
            exitWithCode(ERR_NO_GAME_TO_LAUNCH);
        }
        String jarOp = argList.getFirst();
        argList.removeFirst();
        MinuteEngine engine = new MinuteEngine();
        int ok;
        if ((ok = engine.loadGame(jarOp, argList.toArray(new String[0]))) != NO_ERR) {
            exitWithCode(ok);
        }
        engine.start();
    }
